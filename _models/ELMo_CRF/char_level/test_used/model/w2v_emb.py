# Char-Level, Bi-LSTM, w2v_emb POS 사용 모델
#  Word, POS Tag, char 중 word, Pos Tag만 w2v embedding 수행하고, char는 index embedding한 모델
...
model_save_file_path = absolute_path + '_save/Elmo-CRF/char_level/test_used/model/w2v_emb/'
data_save_file_path = absolute_path + '_save/Elmo-CRF/char_level/test_used/data/w2v_emb/'
embedding_save_file_path = absolute_path +'_save/embeddings/' + inputData + '/w2v_emb/'
model_save = model_save_file_path + inputData + "/" + inputData + '_' + str(epochs) + "_w2v_emb.json"
model_save_weights = model_save_file_path + inputData + "/" + inputData + '_' + str(epochs) + "_w2v_emb.h5"
...
char_to_index, index_to_char = index.make_char_index(train_vocab, 'char')
...
train_sentence_char_idx = index.index_chars(train_sentences, char_to_index, max_len, max_len_char)
...
print("train_sentence_char_idx: ", train_sentence_char_idx[:2], '\n')
...
test_sentence_char_idx = index.index_chars(test_sentences, char_to_index, max_len, max_len_char)
...
print("test_sentence_char_idx: ", test_sentence_char_idx[:2], '\n')
...
X_train_chars = splitPad.get_sublist(train_sentence_char_idx, train_idx)
X_test_chars = splitPad.get_sublist(test_sentence_char_idx, test_idx)
print(X_train_sents)
...
char_embed = TimeDistributed(Embedding(input_dim=CHAR_VOCAB, output_dim=20, input_length=max_len_char, name='char_embedding', mask_zero=False))(char_input)
