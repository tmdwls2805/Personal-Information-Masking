# Char-Level, Bi-LSTM, glove_emb POS 사용 모델
# Word, POS Tag, char 모두 glove embedding 수행한 모델
...
model_save_file_path = absolute_path + '_save/Elmo-CRF/char_level/test_used/model/glove_emb/'
data_save_file_path = absolute_path + '_save/Elmo-CRF/char_level/test_used/data/glove_emb/'
...
if not os.path.exists(vocab_paths):
    ...
    glove_cvocab = glove.load_vocab(char_vocab_paths)
    glove_cmodel = glove.load_model(char_embedding_paths)
...
for char in char_to_index.keys():
    # get the word vector from the embedding model
    # if it's there (check against vocab list)
    if char in glove_cvocab.values():
        # get the word vector
        for key, value in glove_cvocab.items():
            if value == char:
                temp = key
        # get the word vector
        char_vector = glove_cmodel[temp]
        # slot it in at the proper index
