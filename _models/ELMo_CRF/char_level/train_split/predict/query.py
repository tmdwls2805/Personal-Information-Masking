...
def index_sents(sents, vocab, testing=0):
    ...
    for sent in sents:
        sent_vect = []
        for word in sent:
            if word in vocab.item().keys():
                idx = vocab.item()[word]
                sent_vect.append(idx)
            else: # out of max_vocab range or OOV
                sent_vect.append(vocab.item()['OOV'])
    ...
    for sent in sents:
        ...
        for i in range(max_len):
            ...
            for j in range(max_len_char):
                try:
                    if sent[i][j] in char.item().keys():
                        word_seq.append(char.item().get(sent[i][j]))
                    else:
                        word_seq.append(char.item().get('OOV'))
                except:
                    word_seq.append(char.item().get('PAD'))
            sent_seq.append(word_seq)
        vectors.append(np.array(sent_seq))
    return (vectors)


def elmo_crf_char_pred(absolute_path, train_method, model_name, level, emb, txt):

    config = Config(
        json_path=absolute_path + "_models/" + model_name + "/" + level + "/" + train_method + "/config.json")

    absolute_path = config.absolute_path
    load_data = config.training_data
    epochs = config.epochs

    model_save_file_path = absolute_path + '_save/Elmo-CRF/char_level/' + train_method + '/model/' + emb + '/'
    data_save_file_path = absolute_path + '_save/Elmo-CRF/char_level/' + train_method + '/data/' + emb + '/'
    ...
    char_to_index = data_save_file_path + load_data + "/" + load_data + '_' + str(epochs) + "_char_to_index.npy"
    ...
    loaded_char_to_index = np.load(char_to_index, allow_pickle=True)
    ...
    for new_sentence in sentences:
        ...
        for word in me.pos(new_sentence):
        ...
        sentence_text_idx = index_sents([sentence], loaded_word_to_index)
        sentence_pos_idx = index_sents([pos], loaded_pos_to_index)
        sentence_char_idx = index_chars([sentence], loaded_char_to_index, max_len, max_len_char)

        from keras.preprocessing import sequence
        max_len = config.maxlen
        test_sent = sequence.pad_sequences(sentence_text_idx, maxlen=max_len, truncating='post', padding='post')
        test_pos = sequence.pad_sequences(sentence_pos_idx, maxlen=max_len, truncating='post', padding='post')

        this_pred = loaded_model.predict([test_sent, test_pos, np.array(sentence_char_idx)
                                         .reshape((len(sentence_char_idx), max_len, max_len_char))])

        y_predicted = np.argmax(this_pred[0], axis=-1)  # 원-핫 인코딩을 다시 정수 인코딩으로 변경함.
        ...
        for w, t, pred in zip(sentence, test_sent[0], y_predicted):
            if t != 0:  # PAD값은 제외함.
                writer.write(w + "\t")
        ...
        for t, pred in zip(test_sent[0], y_predicted):
            if t != 0:
                tmp_tag_list.append(loaded_index_to_ner.item()[pred])

        for token, t, pred in zip(split_sentence, test_sent[0], y_predicted):
            h = me.pos(token)
            end = len(h) + start
    ...
    rows_rrn.append(rows_rrn_txt), rows_tel.append(rows_tel_txt), rows_date.append(rows_date_txt), rows_mail.append(
        rows_mail_txt)
