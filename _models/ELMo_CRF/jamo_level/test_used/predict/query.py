...
from _utils.jamo.split_char import split_char_convert
            ...
            for j in range(max_len_char):
                try:
                    for jamo in split_char_convert(sent[i][j]):
                        word_seq.append(char.item().get(jamo))
...
def elmo_crf_jamo_pred(absolute_path, train_method, model_name, level, emb, txt):
    config = Config(
    ...
    model_save_file_path = absolute_path + '_save/Elmo-CRF/jamo_level/' + train_method + '/model/' + emb + '/'
    data_save_file_path = absolute_path + '_save/Elmo-CRF/jamo_level/' + train_method + '/data/' + emb + '/'
    ...
    jamo_to_index = data_save_file_path + load_data + "/" + load_data + '_' + str(epochs) + "_jamo_to_index.npy"
    ...
    loaded_jamo_to_index = np.load(jamo_to_index, allow_pickle=True)
    ...
    for new_sentence in sentences:
        ...
        sentence_char_idx = index_jamos([sentence], loaded_jamo_to_index, max_len, max_len_char)
        ...
        this_pred = loaded_model.predict([test_sent, test_pos, np.array(sentence_char_idx)
                                         .reshape((len(sentence_char_idx), max_len, max_len_char * 3))])
