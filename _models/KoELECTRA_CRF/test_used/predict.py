...
from _embeddingModels.koelectra.data_processing import raw_processing
from _embeddingModels.koelectra.trainer import load_model, predict
from _utils.koelectra.utils import init_logger, set_seed
from _parser.parser_sentence import pred_data_parsing
import kss


def koelectra_crf_pred(absolute_path, model_name, train_method, txt):
    # *** Prediction ***
    set_seed()
    init_logger()
    ...
    model, unique_labels, _ = load_model(absolute_path + "_models/" + model_name + "/" + train_method +
                                         './model/koelectra-crf_{}_{}.pth'.format(config.epochs, config.data_name),
                                         config=config,
    ...
    result, rows, cols = predict(pad_data, model, unique_labels,
                                 out_file_name=absolute_path + '_models/' + model_name + '/' + train_method +
                                               './result/raw_prediction_{}_{}.csv'.format(config.epochs,
                                                                                          config.data_name))
