...
from _embeddingModels.bert.data_processing import raw_processing
from _embeddingModels.bert.trainer import load_model, predict
from _utils.bert.utils import init_logger, set_seed
from _parser.parser_sentence import pred_data_parsing
import kss


def bert_crf_pred(absolute_path, model_name, train_method, txt):
    # *** Prediction ***
    set_seed()
    init_logger()
    ...
    model, unique_labels, _ = load_model(absolute_path + "_models/" + model_name + "/" + train_method +
                                         '/model/bert-crf_{}_{}.pth'.format(config.epochs, config.data_name),
                                         config=config,
    ...
    txt, rows, cols = predict(pad_data, model, unique_labels,
