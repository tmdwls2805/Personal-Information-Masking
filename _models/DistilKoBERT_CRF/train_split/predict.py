...
from _embeddingModels.distilkobert.data_processing import raw_processing
from _embeddingModels.distilkobert.trainer import load_model, predict
from _utils.distilkobert.utils import init_logger, set_seed
from _parser.parser_sentence import pred_data_parsing
import kss


def distilkobert_crf_pred(absolute_path, model_name, train_method, txt):
    # *** Prediction ***
    set_seed()
    init_logger()
    ...
    model, unique_labels, _ = load_model(absolute_path + "_models/" + model_name + "/" + train_method +
                                         './model/distilkobert_{}_{}.pth'.format(config.epochs, config.data_name),
                                         config=config,
