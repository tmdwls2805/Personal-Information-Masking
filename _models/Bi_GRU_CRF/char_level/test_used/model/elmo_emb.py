...
model_save_file_path = absolute_path + '_save/Bi-GRU-CRF/char_level/test_used/model/elmo_emb/'
data_save_file_path = absolute_path + '_save/Bi-GRU-CRF/char_level/test_used/data/elmo_emb/'
...
history = model.fit([X_train_sents, X_train_pos, np.array(X_train_chars).reshape((len(X_train_chars), max_len, max_len_char))],
                    y_train_ner, batch_size=config.batch_size, epochs=config.epochs, validation_split=config.validation_rate, verbose=1)
