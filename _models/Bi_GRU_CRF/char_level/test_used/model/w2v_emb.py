# Char-Level, Bi-GRU, w2v_emb POS 사용 모델 (exobrain test-data 사용하여 평가)
# Word, POS Tag, char 모두 w2v embedding 수행한 모델
...
model_save_file_path = absolute_path + '_save/Bi-GRU-CRF/char_level/test_used/model/w2v_emb/'
data_save_file_path = absolute_path + '_save/Bi-GRU-CRF/char_level/test_used/data/w2v_emb/'
...
if not os.path.exists(vocab_paths):
    ...
    w2v_cvocab, _ = word2vec.load_vocab(char_vocab_paths)
    w2v_cmodel = word2vec.load_model(char_embedding_paths)
...
for char in char_to_index.keys():
    # get the word vector from the embedding model
    # if it's there (check against vocab list)
    if char in w2v_cvocab:
        # get the word vector
        char_vector = w2v_cmodel[char]
        # slot it in at the proper index
