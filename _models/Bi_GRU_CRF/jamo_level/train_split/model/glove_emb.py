# Char-Level, Bi-GRU, glove_emb POS 사용 모델
...
inputData = config.traning_data
train_data_path = input_path + inputData

# 모델, weight, 데이터 저장 파일 경로
model_save_file_path = absolute_path + '_save/Bi-GRU-CRF/jamo_level/train_split/model/glove_emb/'
data_save_file_path = absolute_path + '_save/Bi-GRU-CRF/jamo_level/train_split/data/glove_emb/'
...
if not os.path.exists(vocab_paths):
    ...
    glove_jvocab = glove.load_vocab(jamo_vocab_paths)
    glove_jmodel = glove.load_model(jamo_embedding_paths)
...
for jamo in jamo_to_index.keys():
    # get the word vector from the embedding model
    # if it's there (check against vocab list)
    if jamo in glove_jvocab.values():
        # get the word vector
        for key, value in glove_jvocab.items():
            if value == jamo:
                temp = key
        # print("key:",  temp)
        # get the word vector
        jamo_vector = glove_jmodel[temp]
        # slot it in at the proper index
        # print(word, word_vector)
        # print("word_vector:", word, word_vector)
