# Char-Level, Bi-GRU, w2v_emb POS 사용 모델
...
model_save_file_path = absolute_path + '_save/Bi-GRU-CRF/jamo_level/train_split/model/w2v_emb/'
data_save_file_path = absolute_path + '_save/Bi-GRU-CRF/jamo_level/train_split/data/w2v_emb/'
...
if not os.path.exists(vocab_paths):
    ...
    w2v_jvocab, _ = word2vec.load_vocab(jamo_vocab_paths)
    w2v_jmodel = word2vec.load_model(jamo_embedding_paths)
...
for jamo in jamo_to_index.keys():
    # get the word vector from the embedding model
    # if it's there (check against vocab list)
    if jamo in w2v_jvocab:
        # get the word vector
        jamo_vector = w2v_jmodel[jamo]
        # slot it in at the proper index
