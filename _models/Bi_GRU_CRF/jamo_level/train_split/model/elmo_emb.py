# Char-Level, Bi-GRU, elmo_emb POS 사용 모델
...
model_save_file_path = absolute_path + '_save/Bi-GRU-CRF/jamo_level/train_split/model/elmo_emb/'
data_save_file_path = absolute_path + '_save/Bi-GRU-CRF/jamo_level/train_split/data/elmo_emb/'
...
embedding_jamo_weight_paths = pretrain_ckpt_jamo_file_path + 'jamo_weights.hdf5'
...
jamo_to_index, index_to_jamo = index.make_jamo_index(vocab, 'jamo')
...
sentence_jamo_idx = index.index_jamos(sentences, jamo_to_index, max_len, max_len_char)
...
print("train_sentence_jamo_idx: ", sentence_jamo_idx[:2], '\n')
...
X_train_jamos = splitPad.get_sublist(sentence_jamo_idx, train_idx)
X_test_jamos = splitPad.get_sublist(sentence_jamo_idx, test_idx)
print(X_train_sents)
print(X_train_jamos)
...
elmo_jamo_model = ElmoEmbedder(options_file=pretrain_ckpt_jamo_file_path  + "options.json", weight_file=embedding_jamo_weight_paths)
...
for jamo in jamo_to_index.keys():
    # get the word vector
    elmo_jamo_embedding = elmo_jamo_model.embed_sentence(jamo)
    avg_elmo_jamo_embedding = np.average(elmo_jamo_embedding, axis=0)
    jamo_embedding_matrix[jamo_to_index[jamo]] = avg_elmo_jamo_embedding[0]

    # Some progress info
    jamoIdx += 1
    percent = 100.0 * jamoIdx / jamo_to_index.__len__()
    line = '[{0}{1}]'.format('=' * int(percent / 2), ' ' * (50 - int(percent / 2)))
    status = '\r{0:3.0f}%{1} {2:3d}/{3:3d} jamo_tags'
    sys.stdout.write(status.format(percent, line, jamoIdx, jamo_to_index.__len__()))
