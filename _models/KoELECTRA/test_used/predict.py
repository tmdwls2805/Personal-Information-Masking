...
from _embeddingModels.koelectra.trainer_noCRF import load_model, predict
from _utils.koelectra.utils import init_logger, set_seed
from _parser.parser_sentence import pred_data_parsing
import kss


def koelectra_pred(absolute_path, model_name, train_method, txt):
    # *** Prediction ***
    set_seed()
    init_logger()
    ...
    model, unique_labels, _ = load_model(absolute_path + "_models/" + model_name + "/" + train_method +
                                         './model/koelectra_{}_{}.pth'.format(config.epochs, config.data_name),
                                         config=config,
