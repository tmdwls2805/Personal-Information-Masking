...
if not os.path.exists(pretrain_ckpt_char_file_path):
    print("pretrain_ckpt_char_file_path is not exist")
    os.makedirs(pretrain_ckpt_char_file_path)
    exit()
...
# char embeddings
with open(embedding_char_text, 'w', encoding='cp949') as f:
    for s in train_sentences:
        chars = [w_i for w in s for w_i in w]
        f.write(' '.join(chars))
        f.write('\n')

token_num = sent_utils.construct_elmo_vocab(embedding_char_text, elmo_char)

n_gpus = 1  # Number of GPU (GPU 개수가 다르면 수정 필요)

train_elmo.main(embedding_char_text, elmo_char, pretrain_ckpt_char_file_path, token_num, n_gpus)
