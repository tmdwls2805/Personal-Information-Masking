...
class Trainer:
    ...
    def train(self):
        ...
        for epochs in trange(epoch, desc="Epoch"):
            ...
            for step, batch in enumerate(self.tr_dataset):
                ...
                token_ids, token_type_ids, attn_masks, labels = batch
    ...
    # 훈련된 model 저장하는 함수
    def save_model(self):
        torch.save(self.model, './model/koelectra_{}_{}.pth'.format(self.config.epochs, self.config.data_name))
...
def predict(padded_raw_data, model, unique_labels, out_file_name):
    ...
    token_ids, token_type_ids, attn_mask, org_tok_map, sorted_idx, original_tokens = padded_raw_data
    ...
    logits = logits.detach().cpu().numpy()
    pred_labels = np.argmax(logits, axis=2).tolist()
    ...
    for i in range(len(sorted_idx)):
        ...
        for j, orig_tok_idx in enumerate(o2m):
            unique_label = unique_labels[pred_labels[pos][orig_tok_idx]]
