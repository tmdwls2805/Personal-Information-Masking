...
class NER_Dataset(data.Dataset):
    ...
    def make_training_inputs(self):
        ...
        pad_token_id = self.tokenizer.pad_token_id # 1:[PAD]

        # DistilKoBERT는 token_type_ids input으로 넣지 않음..
        # 그 외 input들은 bert 및 kobert와 동일
        ...
        masks = [[float(i != 1) for i in ids] for ids in token_ids]

        return token_ids, masks, token_labels
...
def transform_to_tensor_dataset(config, tokens, labels, masks):
...
def generate_training_data(config, tokenizer, mode=None):
    ...
    token_ids, masks, labels = ner_ds.make_training_inputs()

    # test set 사용하지 않을 경우 train set split
    # distilkobert는 token_type_ids 입력하지 않음
    if mode is None:
        ...
        tr_dataloader = transform_to_tensor_dataset(config, tr_inputs, tr_labels, tr_masks)
        val_dataloader = transform_to_tensor_dataset(config, val_inputs, val_labels, val_masks)
    # test set 사용할 경우
    elif mode == "test":
        tr_inputs = token_ids
        tr_labels = labels
        tr_masks = masks
        val_inputs, val_labels, val_masks = generate_test_data(config, tokenizer, tag2idx)
        tr_dataloader = transform_to_tensor_dataset(config, tr_inputs, tr_labels, tr_masks)
        val_dataloader = transform_to_tensor_dataset(config, val_inputs, val_labels, val_masks)
    else:
...
def generate_test_data(config, tokenizer, tag2idx):
    ...
    ner_ds = NER_Dataset(config=config, tag2idx=tag2idx, sentences=sentences, labels=labels, tokenizer=tokenizer)
    val_inputs, val_masks, val_labels = ner_ds.make_test_inputs()

    return val_inputs, val_labels, val_masks
...
def pad(batch):
    ...
    do_pad = lambda x, seqlen: [sample[x] + [1] * (seqlen - len(sample[x])) for sample in batch]  # 1: [PAD] (KobertTokenizer)
    tok_ids = do_pad(0, maxlen)

    attn_mask = [[float(i != 1) for i in ids] for ids in tok_ids]
    # print(attn_mask)
    ...
    return tok_ids, attn_mask, org_tok_map, list(sorted_idx.cpu().numpy()), original_tokens