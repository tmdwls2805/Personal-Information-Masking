#!/usr/bin/env python
from _embeddingModels.glove import evaluate
...
def load_vocab(vocab_paths):
    ...
    with open(vocab_paths, 'rb') as f:
        g_vocab = pickle.load(f)
    return g_vocab

def load_model(embedding_paths):
    # Load model
    with open(embedding_paths, 'rb') as f:
...
def create_embeddings(file_name,
    ...
    #print("sent:", sent)
    vocab = build_vocab(sent)
    #print("vocab:", vocab)
    cooccur = build_cooccur(vocab, sent, window_size=params.get('window'))
    #print("cooccur:", cooccur)
    id2word = evaluate.make_id2word(vocab)
    #print("id2word:", id2word)

    embedmodel = train_glove(vocab, cooccur, vector_size=100, iterations=params.get('iter'))

    # Merge and normalize word vectors
    embedmodel = evaluate.merge_main_context(embedmodel)

    save_model(embedmodel, embeddings_path)
    save_vocab(id2word, vocab_path)
...
def save_vocab(vocab, path):
    with open(path, 'wb') as handle:
        pickle.dump(vocab, handle, protocol=2)
